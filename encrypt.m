function [cipher]=encrypt(bin_msg,K1,K2,K3,K4)
...
enc{4,3}=xor(fun(enc{4,4}),enc{3,2});
%Step no 02
%Swaping Blocks
enc{5,1}=enc{4,2};
enc{5,2}=enc{4,1};
...
enc{6,1}=not(xor(K3,enc{5,1}));
% Using f f_function and swaping tecniques on 16 bit
enc{6,2}=xor(fun(enc{6,1}),enc{5,3});
%performing xnor operation in last 16 bits with K1
enc{6,4}=not(xor(K3,enc{5,4}));
% Using f f_function and swaping tecniques on 16 bit
enc{6,3}=xor(fun(enc{6,4}),enc{5,2});
%Step no 02
%Swaping Blocks
enc{7,1}=enc{6,2};
enc{7,2}=enc{6,1};
...
enc{8,1}=not(xor(K4,enc{7,1}));
%Using f f_function and Xor operation obtain second 16 bit block
enc{8,2}=xor(fun(enc{8,1}),enc{7,3});
%Performing xnor operation on last 16 bit
enc{8,4}=not(xor(K4,enc{7,4}));
%Using f f_function and Xor operation obtain third 16 bit block
...
% cipher=[enc{2,1},enc{2,2},enc{2,3},enc{2,4}];
end